...
public class ClaimedPolicyServiceImpl implements ClaimedPolicyService {
	...
	public List<ResponseTemplate> getClaimPolicyByUser(int userId) {
		...
		for (int i = 0; i < policyList.size(); i++) {
			...
			User user = restTemplate.getForObject("http://localhost:8080/user-service/users/viewUser/" + userId, User.class);
			Policy policy = restTemplate.getForObject(
					"http://localhost:8080/policy-service/policy/viewPolicy/" + claimPolicyDetails.getPolicyId(), Policy.class);
	...
	public List<ResponseTemplate> listOfAllClaimedPolicies() {
		...
		for (int i = 0; i < policyList.size(); i++) {
			...
			User user = restTemplate
					.getForObject("http://localhost:8080/user-service/users/viewUser/" + claimPolicyDetails.getUserId(), User.class);
